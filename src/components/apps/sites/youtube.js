...
const YoutubeSite = ({number}) => {
    ...
    const play = (e) => {
        ...
        document.getElementById(`${e.currentTarget.id}-${number}`).classList.toggle('highlight')
        document.getElementById('yt-player-${number}').scrollIntoView()
    }

    return(
        <Wrapper>
            <div className="playlist-selector">
                <h3 style={{margin: '6px 2px', color: 'black', gridColumn: 'span 2'}}>Select a <img loading={'lazy'} alt="decor" width={'23px'} height={'12px'} src="/images/desktop/Youtube-logo.webp"/> playlist.</h3>
            ...
            <div style={{marginTop: '25px', border: 'outset 3px', background: 'lightgray', padding: '4px 9px 4px 4px'}}>
                ...
                <iframe id={`yt-player-${number}`} style={{aspectRatio: '16/9', border: "inset 3px", outline: "black solid 1px"}} width="100%" height="100%" src={playlist} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen/>
            </div>
        </Wrapper>
    )
}
